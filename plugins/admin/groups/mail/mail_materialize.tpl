    ...
    <div class="row">
        <div class="col s12 xl6">
            ...
            <div class="input-field">
                ...
                <input id="mail" name="mail" value="{$mail}" {if $mailEqualsCN} disabled {/if}> {/render} @
                ...
                <input type='text' id="mail" name="mail" value="{$mail}"> {/render} {/if} {/if}
                <label for="mail">{t}Primary address{/t}{$must}</label> {/if}
            </div>
            ...
            <div class="input-field">
                ...
                <label>{t}Server{/t}</label> {/render} {/if}
            </div>
            ...
            <div class="contingent">
                ...
                <div class="input-field">
                    ...
                    <input id="gosaMailQuota" name="gosaMailQuota" value="{$gosaMailQuota}" type='text'> {/render}
...
<<<<<<< HEAD
        ...
        <div class="col s12 xl6">
            ...
            <div class="input-field">
                <select name="alternates_list[]" multiple title="{t}List of alternative mail addresses{/t}" {if not $gosaMailAlternateAddress}disabled{/if}>
...
>>>>>>> develop
                ...
                <label for="default_permissions">{t}Default permission{/t}</label> {elseif $user == "__member__"}
                <label for="member_permissions">{t}Member permission{/t}</label> {else}
                <input type='text' name='acl_user_{$item.post_name}' value='{$user}'> {/if}
            </div>

            {if !($user == "__anyone__" || $user == "__member__")}
            <button class="btn-small" type='submit' name='remove_acl_user_{$item.post_name}'>{msgPool type=delButton}</button> {/if} {/render} {if $user == "__member__"} {if $show_effective_memeber}
            <button class="btn-small" type='submit' name='show_effective_memeber'>{t}Hide{/t}</button> {else}
            <button class="btn-small" type='submit' name='show_effective_memeber'>{t}Show{/t}</button> {/if} {/if} {if $user == "__member__" && $show_effective_memeber} {foreach from=$Effective item=i key=k}
            <i>{$k}</i> {/foreach} {/if} {/foreach}
        </div>
        {/if}
        ...
        <div class="col s12 xl6">
            ...
            <div class="input-field add">
                {render acl=$gosaMailForwardingAddressACL}
                <input type='text' name="forward_address" value=""> {/render} {render acl=$gosaMailForwardingAddressACL}
                <button class="btn-small" type='submit' name='add_forwarder'>
						{msgPool type=addButton}
					</button> {/render} {render acl=$gosaMailForwardingAddressACL}
    ...
    <div class="row">
        <div class="col s12">
            ...
            <div class="input-field">
                ...
                <input type="checkbox" name="only_local" value="1" {$only_local} title="{t}Select if user can only send and receive inside his own domain{/t}"> {/render}
