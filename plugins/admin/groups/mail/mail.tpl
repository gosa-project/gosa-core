<table style='width:100%; ' summary="{t}Mail settings{/t}">
    ...
    <tr>
            ...
            <table summary="{t}Address and mail server settings{/t}">
                <tr>
                    ...
                    <td>
                        ...
                        <input type='text' disabled size=30 value="{$mail}"> {else} {if $domainSelectionEnabled} {render acl=$mailACL}
                        <input id="mail" name="mail" size=20 maxlength=65 value="{$mail}" {if $mailEqualsCN} disabled {/if}> {/render} @
                        <select name='MailDomain' size=1>
				{html_options values=$MailDomains output=$MailDomains selected=$MailDomain}
			</select> {else} {if $mailEqualsCN}
                        <input type='text' disabled name='dummy' value='{$mail}'> @
                        <input type='text' value="{$MailDomain}" name="MailDomain"> {else} {render acl=$mailACL}
                        <input type='text' id="mail" name="mail" size=35 maxlength=65 value="{$mail}"> {/render} {/if} {/if} {/if}
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        <input type='text' disabled size=30 value="{$gosaMailServer}"> {else} {render acl=$gosaMailServerACL}
        ...
        <option disabled>&nbsp;</option>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input id="gosaMailQuota" name="gosaMailQuota" size="6" align="middle" maxlength="60" value="{$gosaMailQuota}" type='text'> MB {/render}
                    </td>
                </tr>
                {/if} {if $folderTypesEnabled && !$multiple_support}
                <tr>
                    <td>
                        {t}Folder type{/t}
                    </td>
				...
				{foreach from=$AvailableFolderTypes.SUB_CAT item=item key=key}
					...
					{/if}
				{/foreach}
			</select> {image path="images/lists/reload.png"}
        ...
        <td style='padding-left:2px;'>
			...
			{/render}
            <br> {render acl=$gosaMailAlternateAddressACL}
            <input type='text' name="alternate_address" size="30" align=middle maxlength="60" value=""> {/render} {render acl=$gosaMailAlternateAddressACL}
            <button type='submit' name='add_alternate'>{msgPool type=addButton}</button>&nbsp; {/render} {render acl=$gosaMailAlternateAddressACL}
            <button type='submit' name='delete_alternate'>{msgPool type=delButton}</button> {/render}
        </td>
        {/if}
    </tr>
</table>

<hr> {if !$multiple_support}
<table style='width:100%; ' summary="{t}Mail folder configuration{/t}">
    ...
    <tr>
        <td style='width:50%; ' class='right-border'>
		   ...
		   <option disabled>&nbsp;</option>
		  </select> {if !($user == "__anyone__" || $user == "__member__")}
                        <button type='submit' name='remove_acl_user_{$item.post_name}'>{msgPool type=delButton}</button> {/if} {/render} {if $user == "__member__"} {if $show_effective_memeber}
                        <button type='submit' name='show_effective_memeber'>{t}Hide{/t}</button> {else}
                        <button type='submit' name='show_effective_memeber'>{t}Show{/t}</button> {/if} {/if}
                    </td>
                </tr>
                {if $user == "__member__" && $show_effective_memeber} {foreach from=$Effective item=i key=k}
                <tr>
                    <td>&nbsp;&nbsp;<i>{$k}</i></td>
                </tr>
                {/foreach} {/if} {/foreach}
            ...
            <table style='width:100%; ' summary="{t}Advanced mail options{/t}">
                ...
                <tr>
                    <td>
                        ...
                        <input type="checkbox" name="only_local" value="1" {$only_local} title="{t}Select if user can only send and receive inside his own domain{/t}"> {/render} {t}User is only allowed to send and receive local mails{/t}
                    </td>
                </tr>
            </table>
	...
	{/if}
                ...
                <br> {render acl=$gosaMailForwardingAddressACL}
                <input type='text' name="forward_address" size=20 align=middle maxlength=65 value=""> {/render} {render acl=$gosaMailForwardingAddressACL}
                <button type='submit' name='add_forwarder'>{msgPool type=addButton}</button>&nbsp; {/render} {render acl=$gosaMailForwardingAddressACL}
                <button type='submit' name='add_local_forwarder'>{t}Add local{/t}</button>&nbsp; {/render} {render acl=$gosaMailForwardingAddressACL}
                <button type='submit' name='delete_forwarder'>{msgPool type=delButton}</button> {/render}
...
{if $multiple_support}
<input type="hidden" name="multiple_mail_group_posted" value="1"> {/if}