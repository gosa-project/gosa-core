...
<table style='width:100%;' cellspacing=0 summary="{t}Address configuration{/t}">
    <tr>
        <td><label for="mail">{t}Primary address{/t}</label>{$must} {if !$isModifyableMail && $initially_was_account}
            <input type='text' disabled size=30 value="{$mail}"> {else} {if $domainSelectionEnabled}
       ...
       {html_options values=$MailDomains output=$MailDomains selected=$MailDomain}
            ...
            <input type='text' id="mail" name="mail" size=35 maxlength=65 value="{$mail}"> {/if} {/if}
        </td>
    </tr>
    ...
    <tr>
        ...
        <td class='left-border'>
            ...
            <select id="alternates_list" style="width:100%;height:100px;" name="alternates_list[]" size="15" multiple title="{t}List of alternative mail addresses{/t}"> 
