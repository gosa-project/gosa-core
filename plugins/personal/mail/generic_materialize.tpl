<div class="row setting">
    <div class="col s12 xl6">
        ...
        <div class="input-field">
            ...
            <input type='text' id="mail" name="mail" size=20 maxlength=65 value="{$mail}"> {/render} @

            <select name='MailDomain' size=1>
                     {html_options values=$MailDomains output=$MailDomains selected=$MailDomain}
                  </select> {else} {render acl=$mailACL}
            <input type='text' id="mail" name="mail" size=35 maxlength=65 value="{$mail}"> {/render} {/if} {/if}
        ...
        <div class="leave-notification">
            ...
            <div class="row">
                <div class="input-field col s3">
                    ...
                    <label for="gosaVacationStart">{t}from{/t}</label> {/render}
                </div>
                <div class="input-field col s3">
                    {render acl=$gosaVacationMessageACL}
                    <input type="text" id="gosaVacationStop" name="gosaVacationStop" class="datepicker" value="{$gosaVacationStop}">
                    <label for="gosaVacationStop">{t}till{/t}</label> {/render}
                </div>
            </div>
            {/if} {render acl=$gosaVacationMessageACL checkbox=$multiple_support checked=$use_gosaVacationMessage}
            ...
            {$gosaVacationMessage}
         </textarea> {/render} {if $show_templates eq "true"}
            <div class="input-field add">
                {render acl=$gosaVacationMessageACL}
                <select id='vacation_template' name="vacation_template" {if $own_script !="" } disabled {/if} size=1>
                  ...
                  <option disabled>&nbsp;</option>
               </select> {/render} {render acl=$gosaVacationMessageACL}
    ...
    <div class="col s12 xl6">
                ...
                <div class="col s6">
                    <div class="input-field">
                     ...
                     {html_options values=$spamlevel output=$spamlevel selected=$gosaSpamSortLevel}
                        ...
                        <label for="gosaSpamSortLevel">{t}Move mails tagged with SPAM level greater than{/t}</label> {/render}
                    </div>
                </div>
                ...
                <div class="col s6">
                    <div class="input-field">
                     ...
                     <option disabled>&nbsp;</option>
                        ...
                        <label for="gosaSpamMailbox">{t}to folder{/t}</label> {/render}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="forward-messages">
            ...
            <div class="input-field add {if $multiple_support} move-in {/if}">
                {render acl=$gosaMailForwardingAddressACL}
                <input type='text' id='forward_address' name="forward_address" size=20 align="middle" maxlength=65 value=""> {/render} {render acl=$gosaMailForwardingAddressACL}
                <button class="btn btn-small" type='submit' name='add_forwarder' id="add_forwarder">{msgPool type=addButton}</button> {/render} {render acl=$gosaMailForwardingAddressACL}
                <button class="btn btn-small" type='submit' name='add_local_forwarder' id="add_local_forwarder">{t}Add local{/t}</button> {/render} {render acl=$gosaMailForwardingAddressACL}
                <button class="btn btn-small" type='submit' name='delete_forwarder' id="delete_forwarder">{msgPool type=delButton}</button> {/render}
            </div>
        </div>
    </div>
</div>
