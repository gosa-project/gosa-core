...
<p>
    <input type='radio' name='registrationType' value='dontWant' id="registrationType_dontWant" {if $default=="dontWant" } checked {/if}>
    <b><LABEL for="registrationType_dontWant">{t}I do not want to register{/t}</LABEL></b>
    <p style='padding-left:20px;'>
        <!-- Add a descritive text later -->
    </p>

    <input type='radio' name='registrationType' value='registrate' id="registrationType_registrate" {if $default=="registrate" } checked {/if}><b><LABEL for="registrationType_registrate">{t}Register{/t}</LABEL></b>
    <p style='padding-left:20px;'>
    ...
    <p style='padding-left:20px;'>
        ...
        <ul>
            ...
            <li>{t}When the statistics extension is used. GOsa will transmit information about plugins, their usage and the amount of objects present in your ldap database. No sensitive data is transmitted here, just the object type, the action performed,
                cpu usage, memory usage, elapsed time...{/t}</li>
        </ul>
    </p>
</p>
<hr>
...
<table>
    <tr>
        <td>
            <LABEL for="username">{t}Login{/t}</LABEL>
        </td>
    ...
    <tr>
        <td>&nbsp;</td>
        <td>
            <div style="color: red; font-size: small;"><i>{$error}</i></div>
        </td>
    </tr>
</table>
<hr>
