...
class writeexcel_formula
{
    ...
    function parse_formula()
    {
        ...
        $this->_current_char = 0;
        $this->_lookahead    = substr($this->_formula, 1, 1);
    ...
    function _cellToRowcol($cell)
    {
        ...
        for ($i = 0; $i < strlen($col_ref); $i++) {
            $col += (ord(substr($col_ref, $i, 1)) - ord('A') + 1) * pow(26, $expn);
            $expn--;
        }
    ...
    function _advance()
    {
        ...
        if ($i < strlen($this->_formula)) {
            while (substr($this->_formula, 1, 1) == " ") {
                $i++;
            }
            if ($i < strlen($this->_formula) - 1) {
                $this->_lookahead = substr($this->_formula, $i + 1, 1);
            }
            $token = "";
        }
        while ($i < strlen($this->_formula)) {
            $token .= substr($this->_formula, $i, 1);
            if ($i < strlen($this->_formula) - 1) {
                $this->_lookahead = substr($this->_formula, $i + 1, 1);
            } else {
                $this->_lookahead = '';
            ...
            if ($i < strlen($this->_formula) - 2) {
                $this->_lookahead = substr($this->_formula, $i + 2, 1);
            } else {
    ...
    function _match($token)
    {
        switch ($token) {
            ...
            default:
                ...
                elseif (
                    preg_match("/^[A-Za-z0-9_]+(\:[A-Za-z0-9_]+)?\![A-Ia-i]?[A-Za-z][0-9]+$/", $token) and
                    !preg_match("/[0-9]/", $this->_lookahead) and
                ...
                elseif (
                    preg_match("/^[A-Za-z0-9_]+(\:[A-Za-z0-9_]+)?\!([A-Ia-i]?[A-Za-z])?[0-9]+:([A-Ia-i]?[A-Za-z])?[0-9]+$/", $token) and
                    !preg_match("/[0-9]/", $this->_lookahead)
                ) {
                    return $token;
                }
    ...
    function parse($formula)
    {
        $this->_current_char = 0;
        $this->_formula      = $formula;
        $this->_lookahead    = substr($formula, 1, 1);
        $this->_advance();
    ...
    function _func()
    {
        ...
        while ($this->_current_token != ')') {
            if ($num_args > 0) {
                if (
                    $this->_current_token == SPREADSHEET_EXCEL_WRITER_COMA ||
